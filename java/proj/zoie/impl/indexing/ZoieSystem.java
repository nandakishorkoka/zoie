...
public class ZoieSystem<R extends IndexReader,V> extends AsyncDataConsumer<V> implements DataConsumer<V>,IndexReaderFactory<ZoieIndexReader<R>> {
	...
	public long getMinUID() {
		System.out.println("Calculating minimum UID ..");
		long minUID = 0;
		try {
			minUID = _searchIdxMgr.getMinUID();
		} catch (IOException e) {
			log.error("Error while getting minimum UID from search Index Manager: " + e.getMessage());
		}
		return minUID;
	}

	public long getMaxUID() {
		System.out.println("Calculating maximum UID ..");
		long maxUID = 0;
		try {
			maxUID = _searchIdxMgr.getMaxUID();
		} catch (IOException e) {
			log.error("Error while getting maximum UID from search Index Manager: " + e.getMessage());
		}
		return maxUID;
	}
