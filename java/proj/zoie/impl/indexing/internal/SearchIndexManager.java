...
public class SearchIndexManager<R extends IndexReader>{
	...
	public long getMinUID() throws IOException {
		ZoieMultiReader<R> multiReader = (ZoieMultiReader<R>) ZoieIndexReader.open(_dirMgr.getDirectory(), _indexReaderDecorator);
		List<ZoieSegmentReader<R>> segmentReaders = multiReader.getSubZoieReaders();
		long minUID = Long.MAX_VALUE;
		if(segmentReaders == null)
			log.error("Could not load segment readers ...");
		for(ZoieSegmentReader<R> reader : segmentReaders) {
			long[] uidArray = reader.getUIDArray();
			if(uidArray == null)
				continue;
			Arrays.sort(uidArray);
			minUID = Math.min(minUID, uidArray[0]);
		}
		return minUID;
	}
	...
	public long getMaxUID() throws IOException {
		...
		for(ZoieSegmentReader<R> reader : segmentReaders) {
			...
			if(uidArray == null)
				continue;
			Arrays.sort(uidArray);
			maxUID = Math.max(maxUID, uidArray[uidArray.length-1]);
		}
		return maxUID;
	}
